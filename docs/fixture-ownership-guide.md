...
| `scripts/fixtures/release-check/<fixture>/` | OPTC DB release-detector owners | `npm run test:release-check`, release detector replay, and workflow fixture validation | Add a new directory only for a distinct release branch, source-contract canary, or captured upstream failure. Each directory must include the four files named by shared release-check metadata. |
...
| `scripts/fixtures/release-provenance/*` | OPTC DB release and provenance owners | `scripts/release-provenance-report.spec.ts` through `npm run test:release-check` | Keep fixtures synthetic and compact. Include only the release-trigger report, GitHub Release JSON, manifest/seed, and APK digest payload needed to prove provenance pass/fail behavior. |
...
| performance harness fixture builders in `scripts/perf-ability-filters.mjs`, `scripts/perf-explanation-compare.mjs`, `scripts/perf-saved-team-codecs.mjs`, and `scripts/perf-route-load.mjs` | Performance budget owners | browser performance harnesses, codec timing harnesses, import/share hydration timings, route-load cold-start fixtures, and `Performance Budgets` workflow | Keep large generated fixtures local to the harness until another suite needs the same data shape. Store run output as artifacts, not checked-in fixtures. |
