...
| The importer, or a hand-maintained file under `scripts/data/` | `npm run dataset:pipeline -- --check` | `npm run test:import-pipeline` runs the reader tests first, then the check. `npm run dataset:pipeline` rewrites the document | `docs/import-pipeline.json` still lists the importer's stages, and every file in `scripts/data/` is read by something or has a recorded reason why only a person reads it (869f138r4) | Test output only |
