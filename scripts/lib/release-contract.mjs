...
export const AUTOMATIC_OUTPUT_SIGNATURES = Object.freeze([
  ...
  { id: 'dataset-schema-doc', producer: 'scripts/release-and-tag.sh', signature: 'npm run dataset:schema', what: 'docs/dataset-schema.json regenerated from the imported seed, so the dataset-schema lane holds on main after a release that moves the data' },
  { id: 'offline-pack-contract', producer: 'scripts/release-and-tag.sh', signature: 'npm run packs:contract', what: 'docs/offline-pack-contract.json regenerated from the packs the import refreshed' },
  { id: 'measured-figures', producer: 'scripts/release-and-tag.sh', signature: 'check-dataset-measurements.mjs" --app-root "${PROJECT_ROOT}" --write', what: 'the figures source comments quote with [@dataset] markers, moved to the new measurements - the numbers only, never the words' },
