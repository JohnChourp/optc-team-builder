...
describe('AbilityTagSetPickerComponent', () => {
  ...
  it('confirms with one centred Filter button and no Cancel beside it', () => {
    const template = readFileSync(resolve(process.cwd(), 'src/app/shared/ability-tag-set-picker/ability-tag-set-picker.component.html'), 'utf8');
    ...
    expect(template).toContain("(click)=\"cancel(); tagSetPickerModal.dismiss(null, 'cancel')\"");
