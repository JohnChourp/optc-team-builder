...
| Text on a tool or guide page - the four pages under `tools/` and the three under `guides/`, whose words live once, in `src/app/pages/seo-content/seo-content.data.ts`; their titles, descriptions and canonical paths stay in the route registry row above | `npm run test:discoverability` plus `npm run test:ci -- --include src/app/pages/seo-content/seo-content.page.spec.ts --include src/app/app.routes.spec.ts` | Those plus `npm run test:i18n-regression` when a sentence it pins changes, and `npm run build:pages` then `npm run discoverability:verify` when the generated fallback itself is in scope | The page and the static fallback no-JavaScript crawlers read are built from the same record, so a sentence changed in that file reaches both; every tool and guide route has a record and every record has a route; the reader refuses a value that is not a literal rather than dropping it from the fallback; and no text rides back into the route table, where it would cost the entry script about 7 KB again (869f13c5t) | None; the checks read the source, and the generator run writes only to its output directory |
