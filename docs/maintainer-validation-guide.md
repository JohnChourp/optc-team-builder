...
| The seed, or anything that changes the database's tables or columns | `npm run dataset:schema -- --check` | `npm run test:dataset-schema` runs the reader tests first, then the check. `npm run dataset:schema` rewrites the document | `docs/dataset-schema.json` is still the schema the committed seed has - six tables, their columns and types, which columns hold JSON, and that there are no indexes (869f138qt) | Test output only |
