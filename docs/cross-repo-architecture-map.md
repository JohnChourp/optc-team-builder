...
flowchart TD
  ...
  workflow --> artifacts["release-trigger-outcome, upstream-monitor-report, release-detector-status, release-decision-history, release-provenance"]
...
- `release-trigger-outcome`, `upstream-monitor-report`,
  `release-detector-status`, `release-decision-history`, and
  `release-provenance` workflow artifacts for scan-friendly evidence,
  including upstream fetch diagnostics and recent decision trends without raw
