...
describe('SettingsPage', () => {
  ...
  it('builds an inventory preview from an OPTCbx file without mutating state', async () => {
    ...
    await page.onInventoryOptcbxFileSelected(createFileEvent(buildFile('inventory.json', '{}')), {
      value: '',
    } as HTMLInputElement);
  ...
  it('updates the auto team builder worker mode from settings', async () => {
    ...
    await page.onAutoTeamBuilderWorkerModeChange({ detail: { value: 'manual' } } as CustomEvent<{
  ...
  it('updates the manual worker count from settings', async () => {
    ...
    await page.onAutoTeamBuilderManualWorkerCountChange({ detail: { value: 4 } } as CustomEvent<{
...
function createPage() {
  ...
  const userState = {
    ...
    clearAllSavedRumbleTeams: vi.fn().mockImplementation(async () => {
      savedRumbleTeams.set([]);
    }),
