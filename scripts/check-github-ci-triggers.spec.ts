...
describe('check-github-ci-triggers', () => {
  it('accepts manual and scheduled workflows', async () => {
    ...
    const result = inspectCiTriggers({ appRoot, allowlist: [] });
  ...
  it('formats passing and failing results', () => {
    ...
    expect(
      formatCiTriggerResult({
        ok: false,
        findings: [{ workflowPath: '.github/workflows/test.yml', event: 'pull_request', message: 'nope' }],
        checkedWorkflows: [],
      }),
    ).toContain('.github/workflows/test.yml:pull_request - nope');
  });

  it('keeps the shipped allowlist limited to the production deploy workflow', () => {
    expect(APP_CI_TRIGGER_ALLOWLIST.map((entry) => entry.workflowPath)).toEqual([
      '.github/workflows/deploy-pages.yml',
    ]);
  });

  it('allowlists no pull-request event at all', () => {
    const pullRequestEvents = APP_CI_TRIGGER_ALLOWLIST.flatMap((entry) => Object.keys(entry.events)).filter((event) =>
